...
> **⚠ v0.1 스텁 — CapCut 8.x 스키마 샘플 확보 전**
> 이 스킬은 CapCut 8.5.0의 실제 드래프트 JSON 스키마를 **리버스 엔지니어링해야** 실체 구현이 가능하다. 현재 파일은 **뼈대만** 기록하며, 사용자가 제공할 샘플 JSON을 기반으로 필드를 채워 넣는 것이 다음 작업이다.
>
> 샘플 확보 절차는 [knowledge/capcut8-schema.md](../../knowledge/capcut8-schema.md) 참고.
...
1. 사용자 제공 CapCut 8.5.0 샘플 JSON 분석 → `knowledge/capcut8-schema.md` 채우기
2. `transition_hint` → 실제 CapCut 8.x 전환 `effect_id` 매핑 테이블 작성
3. `mood`/`era` → CapCut 8.x 필터·컬러 그레이딩 `preset_id` 매핑
4. UUID 생성·폴더명 규칙 확정 (샘플에서 관찰)
5. 실제 빌드 → CapCut 8.5.0 에서 열기 검증
6. 에러 발생 시 필드별 이진 탐색으로 원인 교정
